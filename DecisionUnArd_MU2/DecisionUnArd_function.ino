...
Position move_to_target_MU2(Position buPosition, Position currentPosition, Position targetPosition, Position obstacles[], int obstacleCount, int& state){
  ...
  Position aimedPosition = aimedTargetPositionCalculator_MU2(targetPosition);
...
Position move_decider_MU2(Position buPosition, Position currentPosition, Position targetPosition, Position obstacles[], int obstacleCount,int mu_info_matrix[GRID_SIZE][GRID_SIZE],int& state, int bu_link){
  ...
  // my info matrix has ay zeros left or not. If I have no zeros left, then my state should change to TARGET_NOT_FOUND
    ...
    obstacles[obstacleCount + 2].x = 7;
    // We add the tile in the other MUs regions that we might move towards into our obstacles to prevent trespassing.
    obstacles[obstacleCount + 2].y = currentPosition.y;
    
    obstacleCount = obstacleCount + 3;
  ...
  // If target is NOT found move to the BU
    ...
    nextMoveResult = move_to_target_MU2(buPosition, currentPosition, targetPosition, obstacles, obstacleCount, state);
    }
    // Move to the BU
    else{
      nextMoveResult = move_to_BU(buPosition, currentPosition, obstacles, obstacleCount);
...
Position aimedTargetPositionCalculator_MU2(Position targetPosition){
  ...
  if(targetPosition.y ==1){
    aimedTargetPosition.y = targetPosition.y + 1;
  }
  else{
    aimedTargetPosition.y = targetPosition.y - 1;
  }
...
bool collision_avoidance_MU2(Position targetPosition, Position obstacles[], int& obstacleCount, int MU1_state, int MU2_state, int MU3_state, Position MU1_NextPosition, Position MU2_NextPosition,Position MU3_NextPosition){

  int distance_to_MU1 = abs(MU1_NextPosition.x - MU2_NextPosition.x) + abs(MU1_NextPosition.y - MU2_NextPosition.y);
  int distance_to_MU3 = abs(MU2_NextPosition.x - MU3_NextPosition.x) + abs(MU2_NextPosition.y - MU3_NextPosition.y);
  bool movement_allowed = 0;
  bool movement_allowed_wrt_1 = 0;
  bool movement_allowed_wrt_3 = 0;

  if(MU1_state == TARGET_REACHED){
    movement_allowed_wrt_1 = 0;
    movement_allowed_wrt_3 = 0;
  }else{
    // CHECK COLLISION W/ MU1
    if(distance_to_MU3 <= 2){
      ...
      if(MU3_state == TARGET_REACHED){
        ...
        // MU2 is in its own search area
        if(3<MU2_NextPosition.x && MU2_NextPosition.x<7){
          if(MU2_state == CURRENTLY_SEARCHING){

            if(MU3_state == CURRENTLY_SEARCHING){
        ...
        }else if (0<MU2_NextPosition.x && MU2_NextPosition.x<4){
          movement_allowed_wrt_3 = 1;
          obstacles[obstacleCount].x = MU3_NextPosition.x;
          obstacles[obstacleCount].y = MU3_NextPosition.y;  
          obstacleCount++;               
    ...
    if(distance_to_MU1 <= 2){
      if(MU1_state == TARGET_REACHED){
        ...
        if(0<MU2_NextPosition.x && MU2_NextPosition.x<4){
        ...
        }else if(3<MU2_NextPosition.x && MU2_NextPosition.x<7){
          if(MU1_state == CURRENTLY_SEARCHING && MU2_state == CURRENTLY_SEARCHING){
            movement_allowed_wrt_1 = 1;
            obstacles[obstacleCount].x = MU1_NextPosition.x;
            obstacles[obstacleCount].y = MU1_NextPosition.y;  
            obstacleCount++;               
  ...
  // Serial.print("Move allowed 1 ");
  // Serial.println(movement_allowed_wrt_1);
  // Serial.print("Move allowed 3 ");
  // Serial.println(movement_allowed_wrt_3);

  movement_allowed = movement_allowed_wrt_1 && movement_allowed_wrt_3;

  // Serial.print("MOVE IS ALLOWED == ");
