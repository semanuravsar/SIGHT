...
void setup() {
  ...
  //  TO TEST THE BROADCASTING OF TARGET INFO
  // while(1)
  // {
  //   for(int i = 0; i<8; i++){
  //   set_active_s(i);
  //   send_target_info(1, 4, 7, 2); 
  // }
...
if(state == TARGET_REACHED){
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    // Serial.println("Currrent Position read");
...
if(ALERT_BU){
  ...
  for(int i = 0; i<8; i++){
    set_active_s(i);
    send_target_info(currentPosition.x, currentPosition.y, targetPosition.x, targetPosition.y); 
  }
  }
}
