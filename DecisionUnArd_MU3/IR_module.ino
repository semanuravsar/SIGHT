...
//ping_test() yerine
void send_ping_message(Position MU3_NextPosition, Position targetPosition, int ID, uint8_t MU3_state){
  
  uint8_t nextPosition_x = MU3_NextPosition.x;
  uint8_t nextPosition_y = MU3_NextPosition.y;
  ...
  uint8_t second_byte = MU3_state*16 + ID; 
...
void send_ack_message(Position MU3_NextPosition, Position targetPosition, int ID, uint8_t MU3_state){
  ...
  uint8_t first_byte = nextPosition_x*16 + nextPosition_y; 
  uint8_t second_byte = (MU3_state+9)*16 + ID; 
  uint8_t third_byte = targetPosition_x*16 + targetPosition_y;
...
void listener(Position& MU1_NextPosition, Position& MU2_NextPosition, Position& MU3_NextPosition, Position& targetPosition,uint8_t& MU1_state,uint8_t& MU2_state, uint8_t& MU3_state,int& buLink, uint8_t loop_iteration_wo_ping_message){
  ...
  if (listening_result == 1){ // i.e., the message received is succesful
    ...
    Serial.println("x: " + String(received_x) + " y: " + String(received_y) + " intention: " + String(intention) + " received_ID: " + String(received_ID) + " target x: " + String(target_x)+ " target y: " + String(target_y)+ "\n");
    ...
    if(intention == 1){
      ...
      send_ack_message(MU3_NextPosition, targetPosition, ID, MU3_state);  
      send_ack_message(MU3_NextPosition, targetPosition, ID, MU3_state); 
