function [mu1_all,mu2_all,mu3_all,bu,targetPosition,i,obstaclePositions] = best_case(mu1,mu2,mu3,bu,targetPosition,obstaclePositions)
gridSize = 9;
...
for i=1:numberofiterations
    ...
    avoidPositions = [buPosition; obstaclePositions];
    
    % mu1 plans its move
    trajectory_mu1 = trajectory_maker(currentPosition1, targetPosition);
    %fix the trajectory

    for j = 1:size(avoidPositions, 1)
        obstaclePosition = avoidPositions(j,:);
        trajectory_mu1 = trajectory_fixer(trajectory_mu1, currentPosition1, obstaclePosition, gridSize) ;      
    end
    ...
    trajectory_mu2 = trajectory_maker(currentPosition2, targetPosition);
    %fix the trajectory

    for j = 1:size(avoidPositions, 1)
        obstaclePosition = avoidPositions(j,:);
        trajectory_mu2 = trajectory_fixer(trajectory_mu2, currentPosition2, obstaclePosition, gridSize) ;           
    end
    ...
    % mu3 plans its move    
    trajectory_mu3 = trajectory_maker(currentPosition3, targetPosition);
     %fix the trajectory

    for j = 1:size(avoidPositions, 1)
        obstaclePosition = avoidPositions(j,:);
        trajectory_mu3 = trajectory_fixer(trajectory_mu3, currentPosition3, obstaclePosition, gridSize) ;
    end
    ...
    if([nextPosition1(1), nextPosition1(2)] == [targetRow,targetCol])
    ...
    % check if already at target, if so make next position current position
    if([nextPosition2(1), nextPosition2(2)] == [targetRow,targetCol])
        ...
        currentPosition2 = nextPosition2;
        %give the information of the location of where the mu is.
        mu2_info(nextPosition2(1), nextPosition2(2))=1;        
    end
    
    % check if already at target, if so make next position current position
    if([nextPosition3(1), nextPosition3(2)] == [targetRow,targetCol])
        currentPosition3 = nextPosition3;
        mu1_info(currentPosition3(1), currentPosition3(2))=6;
        targetisFound3 = 1;
        break
    else     
        %Change the value of currentPosition as scanned
        ...
        currentPosition3 = nextPosition3;
        %give the information of the location of where the mu is.
        mu1_info(nextPosition3(1), nextPosition3(2))=1;        
    end
