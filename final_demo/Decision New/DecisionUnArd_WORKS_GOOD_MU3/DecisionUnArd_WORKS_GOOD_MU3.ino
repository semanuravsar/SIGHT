...
Position MU2_NextPosition = {10, 11};
...
// // Next pos to be used in the comm protocol
// Position MU1_NextPosition = {11, 13};
// Position MU2_NextPosition = {5, 6};
// Position MU3_NextPosition = {5, 6};
...
uint8_t move_decider_wo_comm() {
  ...
  if(state ==  TARGET_FOUND && buLink && (currentPosition.x == aimedTargetPosition.x && currentPosition.y == aimedTargetPosition.y) ){
    ...
    MU3_NextPosition.x = currentPosition.x;
    MU3_NextPosition.y = currentPosition.y;
  ...
  // Call move_decider_MU3
  Position nextMoveResult = move_decider_MU3(buPosition, currentPosition, targetPosition, obstacles, obstacleCount, mu_info_matrix, state, buLink);
  MU3_NextPosition.x = currentPosition.x + nextMoveResult.x;
  MU3_NextPosition.y = currentPosition.y + nextMoveResult.y;
...
movement_allowed = collision_avoidance_MU3(targetPosition, obstacles, obstacleCount, MU1_state, MU2_state, MU3_state, MU1_NextPosition, MU2_NextPosition, MU3_NextPosition); // NEW
    ...
    if(currentPosition.x == -1){
      ...
      if(!movement_allowed){
        ...
        while(!movement_allowed){
          ...
          final_communication(MU1_NextPosition, MU2_NextPosition, MU3_NextPosition, targetPosition, ID, MU1_state, MU2_state, MU3_state, buLink, active_s);
          movement_allowed = collision_avoidance_MU3(targetPosition, obstacles, obstacleCount, MU1_state, MU2_state, MU3_state, MU1_NextPosition, MU2_NextPosition, MU3_NextPosition); 
...
if(ALERT_BU){
  ...
  for(int i = 0; i<8; i++){
    set_active_s(i);
    send_ping_message(MU3_NextPosition, targetPosition, ID, state);  }
  }
}
