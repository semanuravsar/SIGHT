...
//ping_test() yerine
void send_ping_message(Position buPosition, Position targetPosition, int ID, uint8_t MU1_state){  
  
  uint8_t nextPosition_x = 5;
  uint8_t nextPosition_y = 5;
  ...
  uint8_t second_byte = MU1_state*16 + ID; 
...
void send_ack_message(Position buPosition, Position targetPosition, int ID, uint8_t MU1_state) {
  ...
  uint8_t first_byte = nextPosition_x*16 + nextPosition_y; 
  uint8_t second_byte = (MU1_state+9)*16 + ID; 
  uint8_t third_byte = targetPosition_x*16 + targetPosition_y;
...
void final_communication_before_target(Position& MU1_NextPosition, Position& MU2_NextPosition, Position& MU3_NextPosition, Position& targetPosition, int ID, uint8_t& MU1_state, uint8_t& MU2_state, uint8_t& MU3_state, int& buLink, uint8_t& active_s){
  ...
  listener_before_target(MU1_NextPosition, MU2_NextPosition, MU3_NextPosition, targetPosition,MU1_state,MU2_state, MU3_state, buLink, loop_iteration_wo_ping_message);
...
void listener(Position& MU1_NextPosition, Position& MU2_NextPosition, Position& MU3_NextPosition, Position& targetPosition,uint8_t& MU1_state,uint8_t& MU2_state, uint8_t& MU3_state, int& buLink, uint8_t loop_iteration_wo_ping_message){
  ...
  if (listening_result == 1){ // i.e., the message received is succesful
    ...
    if(received_ID == 2){
      MU2_NextPosition.x = received_x;
      MU2_NextPosition.y = received_y;
      MU2_state = received_state;
    ...
    if(intention == 1){
      ...
      send_ack_message(buPosition, targetPosition, ID, 4);  
      send_ack_message(buPosition, targetPosition, ID, 4); 
...
void listener_before_target(Position& MU1_NextPosition, Position& MU2_NextPosition, Position& MU3_NextPosition, Position& targetPosition,uint8_t& MU1_state,uint8_t& MU2_state, uint8_t& MU3_state, int& buLink, uint8_t loop_iteration_wo_ping_message){
  ...
  if (listening_result == 1){ // i.e., the message received is succesful
    ...
    if(intention == 1){
      ...
      loop_iteration_wo_ping_message = 0;
      // // Serial.println("Ping PACKAGE is received, sending ping ack");
      // send_ack_message(MU1_NextPosition, targetPosition, ID, MU1_state);  
      // send_ack_message(MU1_NextPosition, targetPosition, ID, MU1_state); 
