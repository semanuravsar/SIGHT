...
float line_position = -99;   
unsigned long line_found_when = 0; 
uint8_t line_following = 0;
uint8_t is_black[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };  // if i'th sensor is black, set to 1. otherwise 0
uint8_t hard_break_period = 0;
uint8_t stop_period = 0;
unsigned long hard_break_enter_time = 0;
unsigned long stop_enter_time = 0;
uint8_t line_crossing = 0;
uint8_t find_rfid = 0;
unsigned long rfid_search_start_time = 0;
unsigned long last_line_crossing_time = 0;
float BASE_SPEED;
float del_speed;
float M1_speed;
float M2_speed;
...
void loop() {
  ...
  // if not at a crossing, follow the line
  if (number_of_blacks > 0 && number_of_blacks < 4) {  // Atleast one sensor detects black.
  line_found_when = millis();
  line_position = 0;
  for (uint8_t i = 0; i < 8; i++) {
    if (is_black[i] == 1) line_position += (i - 4);
  }
  line_position = line_position / float(number_of_blacks);  // Ensure that this is a float division

  BASE_SPEED = 0.20;                 //speed that each motor should go with when the line is centered in m/s
  del_speed = line_position * 0.03;  // m/s
  M1_speed = BASE_SPEED - del_speed;
  M2_speed = BASE_SPEED + del_speed;
  ...
  // at line crossing, first stop, then move untill rfid card is read
  else if (number_of_blacks >= 4) {  //line crossing is found, STOP
    find_rfid = 1;
    line_crossing = 1;
    last_line_crossing_time = millis();
    if (line_following == 1){ // first time entering here
      hard_break_period = 1;
      stop_period = 0;
      hard_break_enter_time = millis();
    }
    line_following = 0;
    if (millis() - hard_break_enter_time > 75 && hard_break_period == 1){
      hard_break_period = 0;
      stop_period = 1;
      stop_enter_time = millis();
    }
    if (millis() - stop_enter_time > 500 && stop_period == 1){
      stop_period = 0;
    }
    if (hard_break_period ==1){
      set_motor_speeds(-0.05, -0.05);
    }else if (stop_period == 1){
  ...
  if (millis()-last_line_crossing_time > 1500 && find_rfid == 1){
    rfid_search_start_time = millis();
    unsigned long last_direction_change_time = millis();
    int direction = -1;

    while(millis() - rfid_search_start_time < 10000 ){
      pin_val = digitalRead(RFID_READ);
      if (pin_val == 1){
        Serial.println("RFID found while git-gel");
        find_rfid = 0;
        break;
      }

      if (millis()-last_direction_change_time > 1500){
      ...
      for (uint8_t i = 0; i < 8; i++) {
        ...
        BASE_SPEED = 0.20;                 //speed that each motor should go with when the line is centered in m/s
        del_speed = line_position * 0.03;  // m/s
        M1_speed = BASE_SPEED -1*direction* del_speed;
        M2_speed = BASE_SPEED +1*direction* del_speed;
        if (M1_speed < 0.14) M1_speed = 0.14;
        if (M2_speed < 0.14) M2_speed = 0.14;

        set_motor_speeds(M1_speed*direction, M2_speed*direction);
        line_following = 1;
        }
        // at line crossing, first stop, then move untill rfid card is read
        else if (number_of_blacks >= 4) {  //line crossing is found  
          set_motor_speeds(0.19*direction, 0.19*direction);
        }    
  ...
  if (Serial.available() > 0) {  // Check if data is available to read
      ...
      uint8_t decision_unit_command = Serial.read();
      // decision_unit_command = decision_unit_command - 48;

      if (1) { //decision_unit_command != oldData
        ...
        if ((decision_unit_command >= 0 || decision_unit_command < 14)) {
          // delay(100);
          digitalWrite(buzzerPin, HIGH);  // Turn the buzzer on
